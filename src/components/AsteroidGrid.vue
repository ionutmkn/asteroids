<template>
    <div class="card mt-5"> 
        ...
        <transition name="shooting-star"> 
            <div class="mt-3 ml-3" v-cloak v-if="numAsteroids && showSummary"> 
                <p>showing {{numAsteroids}} items</p> 
                <p>{{closestObject}} has the smallest miss distance.</p> 
            </div> 
        </transition> 
        <div class="m-3"> 
            <button class="btn btn-success" @click="showSummary = !showSummary">Show/hide summary</button> 
        </div> 
                ...
                <tr v-for="(a, index) in asteroids" 
                    ...
                    <td> 
                        <ul v-if="a.close_approach_data.length > 0"> 
                            <li :key="key" v-for="(value, key) in a.close_approach_data[0].miss_distance"> 
                                {{key}}: {{value}} 
                            </li> 
                        </ul> 
                    </td> 
